...
class Part4Problem7(Slide, DistributionTemplate):
    ...
    def construct(self):
        ...
        sq1 = self.reveal_term(r"\sqrt{(x-1)^2}", after=None, source=f1, color=SQUARE_COLOR)
        sq_eq = self.reveal_term(r"=", after=sq1, indicate=False)
        sq2 = self.reveal_term(r"\pm i\sqrt{2.5}", after=sq_eq, source=f2)
        self.finish_line([sq1, sq_eq, sq2])

        # --- evaluate the square root ---
        ev1 = self.reveal_term(r"x-1", after=None, source=sq1, indicate=False)
        ev_eq = self.reveal_term(r"=", after=ev1, indicate=False)
        ev2 = self.reveal_term(r"\pm 1.581i", after=ev_eq, source=sq2)
        self.finish_line([ev1, ev_eq, ev2])

        # --- isolate x ---
        iso_line = self.reveal_term(r"x = 1 \pm 1.581i", after=None, source=None, indicate=False)
