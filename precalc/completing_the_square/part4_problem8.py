...
class Part4Problem8(Slide, DistributionTemplate, FrequencyTableTemplate):
    """Part 4, Problem 8 (YOU DO): 4x^2 + 10x + 8 = 4x + 3 -> x = -0.75 +/- i*sqrt(0.6875)."""
    ...
    def _original(self):
        original = MathTex(r"{{4x^2}}{{+10x}}{{+8}}={{4x}}+{{3}}")
    ...
    def show_practice(self):
        self.show_label("Part 4, Problem 8 (YOU DO)")
    ...
    def show_solution(self):
        self.show_label("Part 4, Problem 8 (Solution)")
        ...
        inv1_lhs, inv1_rhs = self.write_inverse([t_10x, t_4x], r"-4x", color=VAR_COLOR)
        strike1 = self.strike_pair(t_4x, inv1_rhs)
        self.extend_current_line(inv1_lhs, inv1_rhs, strike1)

        # --- move the constant: add -8 to both sides ---
        inv2_lhs, inv2_rhs = self.write_inverse([t_8, t_3], r"-8", color=WHITE)
        strike2 = self.strike_pair(t_8, inv2_lhs)
        ...
        m2 = self.reveal_term(r"+6x", after=m1, source=[t_10x, inv1_lhs], color=VAR_COLOR)
        m_eq = self.reveal_term(r"=", after=m2, indicate=False)
        m3 = self.reveal_term(r"-5", after=m_eq, source=[t_3, inv2_rhs])
        ...
        f2 = self.reveal_term(r"-0.6875", after=f_eq, source=[d3, sq_rhs])
