...
class Part2Problem3(Slide, DistributionTemplate):
    """Part 2, Problem 3 (I DO): 2x^2 + 7x - 4 = 4x + 2 -> x = -0.75 +/- sqrt(3.5625)."""

    def construct(self):
        self.camera.background_color = BACKGROUND_COLOR
        self.show_label("Part 2, Problem 3 (I DO)")

        original = MathTex(r"{{2x^2}}{{+7x}}{{-4}}={{4x}}+{{2}}")
        original.get_part_by_tex("2x^2", substring=False).set_color(VAR_COLOR)
        original.get_part_by_tex("+7x", substring=False).set_color(VAR_COLOR)
        ...
        t_7x = original.get_part_by_tex("+7x", substring=False)
        ...
        inv1_lhs, inv1_rhs = self.write_inverse([t_7x, t_4x], r"-4x", color=VAR_COLOR)
        ...
        inv2_lhs, inv2_rhs = self.write_inverse([t_neg4, t_2], r"+4", color=WHITE)
        strike2 = self.strike_pair(t_neg4, inv2_lhs)
        ...
        m2 = self.reveal_term(r"+3x", after=m1, source=[t_7x, inv1_lhs], color=VAR_COLOR)
        m_eq = self.reveal_term(r"=", after=m2, indicate=False)
        m3 = self.reveal_term(r"6", after=m_eq, source=[t_2, inv2_rhs])
        ...
        sqrt_line = self.reveal_term(r"x = -0.75 \pm \sqrt{3.5625}", after=None, source=None, indicate=False)
        self.finish_line([sqrt_line])

        # --- decimal answer ---
        final_line = self.reveal_term(
            r"x \approx 1.137 \text{ or } x \approx -2.637",
